...
class RiskManager:
    ...
    def check_drawdown(self, current_equity: float) -> bool:
        ...
        if drawdown >= self.max_daily_drawdown:
            ...
            self.halt_reason = f"Daily drawdown {drawdown:.1%} >= {self.max_daily_drawdown:.1%} limit"
            return False
        return True

    def calculate_position_size(self, account_equity: float, entry_price: float, atr: float, available_cash: float = None, ticker: str = None) -> dict:
        """
        ...
        return {
            ...
            'risk_reward_ratio': round(config.ATR_TAKE_PROFIT_MULTIPLIER / config.ATR_STOP_MULTIPLIER, 2),
        }

    def calculate_trailing_stop(self, current_price: float, atr: float, current_stop: float, side: str = 'long') -> float:
        """
        ...
        """
        trail_distance = atr * config.ATR_TRAIL_MULTIPLIER
    ...
    def get_risk_status(self, account_equity: float) -> dict:
        """Returns current risk status for the dashboard."""
        drawdown_pct = 0.0
        ...
        return {
            ...
            'max_drawdown_limit': config.MAX_DAILY_DRAWDOWN * 100,
